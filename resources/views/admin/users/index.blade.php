<x-admin-layout title="Data User">
  <x-slot:content-header>
    <div class="flex w-full justify-between items-center">
      ...
      <a href="{{ route('users.create') }}" class="btn btn-sm btn-success justify-end"><i class="ri-add-line"></i>Add</a>
    </div>
  </x-slot:content-header>
  ...
  <div class="overflow-x-auto mt-4 lg:mx-4">
    <table class="table table-zebra">
      ...
      <tbody>
        ...
        <tr>
          <th>{{ $i+1+(($users->currentPage()-1) * 10) }}</th>
          <td>{{ $user->name }}</td>
          <td>{{ $user->email }}</td>
          <td class="text-center">
            <div class="flex justify-center gap-2">
              <div class="tooltip min-h-6 flex items-center" data-tip="Edit">
                <a class="btn btn-xs btn-circle btn-info font-normal" href="{{ route('users.edit', $user) }}">
              ...
              <div class="tooltip min-h-6 flex items-center" data-tip="Delete">
                <button @click="showAlert({
                  type: 'confirm',
                  title: 'Delete User',
                  message: 'Are you sure you want to delete user with name <strong>{{ htmlspecialchars($user->name) }}</strong> ?',
